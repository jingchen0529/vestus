...
## 34. 后端目录重构设计方案（专业 FastAPI 结构）

当前后端为根目录平铺布局：`app.py`（1,282 行，混合 56 个路由、全部 Pydantic 模型与认证逻辑）和 `db.py`（1,395 行，混合 10 个模型、序列化、`Database` 生命周期与全部数据访问）承载了绝大部分职责，是 F-04（同步阻塞）、F-09/F-11（事务边界）、F-17（大模块弱类型）的结构性根因。本章给出目标结构与渐进迁移路径；**本章为设计方案，未改动任何代码**。

### 34.1 设计目标

- 职责分层：路由（api）→ 业务（services）→ 数据访问（repositories）→ 模型（models），DTO（schemas）横跨路由与业务。
- 事务所有权收口到 service 层，repository 只接收 session、不自行 commit（对应 F-09/F-11，与第 33 章 Phase 3 一致）。
- 配置类型化 + fail-fast：全部 `VESTUS_*` 收敛到 typed settings，缺失或占位值拒绝启动（对应 F-01）。
- 行为等价优先：HTTP 契约不变，web / desktop 前端零改动；每个阶段独立可验证、可回滚。

### 34.2 目标目录结构

```
.
├── backend/                       # Python 后端（最终形态；阶段 1–5 期间 app/ 位于仓库根目录）
│   ├── app/
│   │   ├── __init__.py
│   │   ├── main.py                # FastAPI 实例、中间件注册、路由挂载、/admin 静态托管
│   │   ├── core/
│   │   │   ├── config.py          # pydantic-settings：VESTUS_* 类型化 + 启动校验（fail-fast）
│   │   │   ├── security.py        # argon2 口令散列、token、代理口令 Fernet 加解密
│   │   │   ├── uploads.py         # 上传落盘与清理
│   │   │   └── middleware.py      # security headers、上传请求体硬上限 ASGI 中间件
│   │   ├── db/
│   │   │   ├── base.py            # DeclarativeBase 与公共工具（utc_now、ip_bytes 等）
│   │   │   ├── session.py         # engine / sessionmaker / 初始化与连通性检查
│   │   │   └── models/            # 每模型一文件：admin / user / proxy / platform / setting
│   │   ├── schemas/               # Pydantic DTO（请求 + 响应，含校验器）
│   │   │   └── auth / admin / user / proxy / platform / settings
│   │   ├── api/
│   │   │   ├── deps.py            # current_account / admin_auth / super_admin_auth / user_auth
│   │   │   └── routers/           # system、admin_auth、user_auth、admins、users、proxies、
│   │   │                          # platforms、desktop_config、settings、uploads、legacy
│   │   ├── services/              # 业务逻辑，唯一持有事务提交权
│   │   │   └── auth / admin / user / proxy / platform / desktop_config / settings / audit
│   │   └── repositories/          # 纯数据访问，接收 session，不 commit
│   │       └── admin / user / proxy / platform / setting / log
│   ├── migrations/                # Alembic（取代 create_all 与手写 SQL）
│   │   ├── env.py
...
| 现有文件（内容段） | 目标位置 | 说明 |
|---|---|---|
| `app.py`（Pydantic 模型与校验器，约 L100–486） | `app/schemas/*.py` | `_validate_bypass_hosts`、`_validate_platform_url` 随 DTO 迁移 |
| `app.py`（认证辅助与依赖，约 L488–600） | `app/api/deps.py` | token 解析、同源 Cookie 校验 |
| `app.py`（56 个路由） | `app/api/routers/*.py` | 按现有 OpenAPI tag 拆成 11 个 router |
| `app.py`（middleware / 静态挂载 / 启动逻辑） | `app/main.py` + `app/core/middleware.py` | |
| `db.py`（10 个 ORM 模型） | `app/db/models/*.py` | 每模型一文件，`__init__.py` 统一导出 |
| `db.py`（`_*_dict` 序列化，约 15 处） | `app/schemas/`（响应模型）或服务层组装 | 逐步消除 `Dict[str, Any]` 弱类型（F-17） |
| `db.py`（`Database` 类与全部查询方法） | `app/db/session.py` + `app/repositories/*` | 生命周期与数据访问分离 |
| `security.py` | `app/core/security.py` | 内容不变，仅迁移 |
| `file_storage.py` | `app/core/uploads.py` | |
| `upload_middleware.py` | `app/core/middleware.py` | |
| `init_db.py` | `backend/scripts/init_db.py` | 改为 `alembic upgrade head` + 管理员引导 |
| `schema.sql` | 废弃 | 由 Alembic 基线取代，保留于文档说明 |

### 34.4 分层约束

- 依赖方向：`routers → services → repositories → models`；`schemas` 供 `routers` 与 `services` 共用；禁止反向导入。
- 事务规则：仅 `services` 调用 `session.commit()`；`repositories` 全部接收外部传入的 session（对应当前 `db.py` 已有的 `db: Optional[Session]` 参数模式）。
- 约束可执行化：用 import-linter（或等价 CI 检查）固化依赖方向，与桌面端现有 `check:surfaces` 边界检查同一思路。

### 34.5 分阶段迁移计划（每阶段独立可交付）

| 阶段 | 内容 | 同步修复 | 验证 |
|---|---|---|---|
| 0 测试加固 | 为 admins / proxies / platforms 等未覆盖路由补 characterisation tests，锁定现有行为 | — | 新增用例全绿 |
| 1 成包 | 根目录模块 → `app/` 包，入口改 `uvicorn app.main:app`；security/uploads/middleware 移入 `core/` | — | pytest 全绿；更新 README 与 `deploy/*.example` 启动命令 |
| 2 拆 `db.py` | models / session / repositories 三分；Alembic 基线 `0001_baseline`，现有库 `stamp` 后接管 | — | 迁移在真实 MySQL 演练；`create_all` 路径保留为回退 |
| 3 拆 `app.py` | schemas / deps / 11 个 routers；HTTP 契约逐字节不变 | — | OpenAPI schema 前后 diff 为空 |
| 4 提取 `services/` | 事务边界收口；同步修复审计日志原子性与设置半提交；DB I/O handler 去掉 `async`（线程池化） | F-04 / F-09 / F-11 | 故障注入测试：写失败必全回滚 |
| 5 类型化配置 | `pydantic-settings` 收敛全部 `VESTUS_*`，缺失/占位值拒绝启动 | F-01 | 配置矩阵测试 |
| 6（可选）monorepo 归位 | 整体移入 `backend/`，同步 deploy/docs/CI 路径 | — | 部署模板在干净环境验证 |

### 34.6 风险与验收标准

**主要风险：**

- 批量搬迁导致行为漂移 → 一次只移动一个边界，每阶段提交前全部测试绿；阶段 3 用 OpenAPI 快照 diff 锁契约。
- 生产数据库已由 `create_all` 建表 → Alembic 基线生成后先 `stamp` 再增量 `upgrade`，不重建表。
- `os.getenv` 分散读取 → 阶段 5 才集中收敛，期间环境变量名保持完全不变，避免破坏现有部署。
- `uvicorn app:app` → `app.main:app` 的入口变化 → 阶段 1 同步更新 README、`deploy/vestus.service.example` 与部署文档。

**验收标准：**

- 现有 52 项后端测试全绿，并为 services/repositories 补充单测。
- `ruff` / `mypy` 零告警；import-linter 契约通过。
- `uvicorn app.main:app` 正常启动，`/healthz` 通过，OpenAPI schema 与重构前一致。
- web / desktop 零改动完成全部现有流程（登录、配置下发、上传）。
- 缺失或占位 `VESTUS_SECRET_KEY` 时进程拒绝启动。
