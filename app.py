...
def _cors_origins() -> List[str]:
    raw = os.getenv("VESTUS_CORS_ORIGINS", "http://localhost:5173,http://127.0.0.1:5173,tauri://localhost,http://tauri.localhost")
...
class DesktopConfigUpdate(BaseModel):
    proxy_id: Optional[int] = Field(alias="proxyId", ge=1)
    platform_ids: List[int] = Field(alias="platformIds", max_length=1000)
    model_config = ConfigDict(populate_by_name=True, extra="forbid")

    @field_validator("platform_ids")
    @classmethod
    def validate_platform_ids(cls, value: List[int]) -> List[int]:
...
async def current_account(
    ...
    raw = _extract_token(authorization, admin_cookie)
    if not raw:
...
async def product(response: Response) -> Dict[str, str]:
    response.headers["Cache-Control"] = "no-store"
    return {"productName": _product_name()}
...
async def user_desktop_config(
    ...
    if bool(auth["model"].must_change_password):
        raise HTTPException(status_code=403, detail="请先修改管理员下发的临时密码")
...
async def reset_user_password(user_id: int, payload: PasswordReset, request: Request, auth: Dict[str, Any] = Depends(admin_auth)) -> Dict[str, bool]:
    ...
    db.update_user(user_id, {"password": payload.password, "must_change_password": True})
...
async def delete_user_legacy(user_id: int, request: Request, auth: Dict[str, Any] = Depends(admin_auth)) -> Dict[str, bool]:
    existing = db.get_user(user_id)
    if existing is None: raise HTTPException(status_code=404, detail="用户不存在")
...
ADMIN_PAGE = Path(__file__).resolve().parent / "web" / "admin.html"


@app.get("/admin", include_in_schema=False)
async def admin_page() -> Response:
    if not ADMIN_PAGE.exists(): return JSONResponse({"detail": "admin page unavailable"}, status_code=404)
    return FileResponse(ADMIN_PAGE, media_type="text/html; charset=utf-8")
